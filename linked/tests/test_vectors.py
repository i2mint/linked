...
def test_mutual_knn_graph():
    ...
    graph = mutual_knn_graph(
        vectors, n_neighbors=5, approximate=False, ensure_connectivity="none"
    )
    ...
    graph_mst = mutual_knn_graph(
        vectors, n_neighbors=5, approximate=False, ensure_connectivity="mst"
    )
    assert len(graph_mst) >= len(
        graph
    ), "MST version should have at least as many edges"
...
def test_adaptive_knn_graph():
    ...
    assert np.all(graph[:, 2] >= 0) and np.all(
        graph[:, 2] <= 1
...
def test_random_graph():
    ...
    graph_weighted = random_graph(
        20, model="erdos_renyi", p=0.3, weighted=True, seed=42
    )
...
def test_estimators():
    ...
    assert (
        graph_adaptive.shape[1] == 3
    ), "Adaptive KNN estimator should return 3 columns"
...
def test_edge_format():
    ...
    assert np.issubdtype(graph[:, 0].dtype, np.integer) or np.all(
        graph[:, 0] == graph[:, 0].astype(int)
    )
    assert np.issubdtype(graph[:, 1].dtype, np.integer) or np.all(
