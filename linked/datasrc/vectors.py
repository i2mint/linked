...
try:
    ...
    warnings.warn(
        "pynndescent not available. Approximate methods will fall back to exact."
    )
...
def mutual_knn_graph(
    ...
    *,
    ...
    n_samples = vectors.shape[0]
    knn_matrix = _neighbors_to_sparse_matrix(
        neighbors, distances if mode == "distance" else None, n_samples
    )
...
def epsilon_graph(
    ...
    *,
    ...
    if _has_sklearn:
        ...
        return (
            np.array(edges) if edges else np.empty((0, 3 if mode == "distance" else 2))
        )
...
def _watts_strogatz_graph(
    ...
    for edge in edges:
        ...
        if rng.random() < p:
            ...
            possible_targets = [
                t for t in range(n) if t != i and t not in existing_targets[i]
            ]
            if possible_targets:
