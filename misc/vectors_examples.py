...
def example_mutual_knn():
    ...
    graph_mutual = mutual_knn_graph(
        vectors, n_neighbors=10, approximate=False, ensure_connectivity="none"
    )
    ...
    graph_mutual_mst = mutual_knn_graph(
        vectors, n_neighbors=10, approximate=False, ensure_connectivity="mst"
    )
...
def example_adaptive_knn():
    ...
    graph_adaptive = adaptive_knn_graph(
        vectors, n_neighbors=8, local_connectivity=1, bandwidth=1.0, approximate=False
    )
    ...
    print(
        f"Standard weights - mean: {np.mean(graph_standard[:, 2]):.4f}, "
        f"std: {np.std(graph_standard[:, 2]):.4f}"
    )
    print(
        f"Adaptive weights - mean: {np.mean(graph_adaptive[:, 2]):.4f}, "
...
def example_random_graphs():
    ...
    graph_weighted = random_graph(
        n_nodes, model="erdos_renyi", p=0.1, weighted=True, seed=42
    )
...
def example_metrics():
    ...
    for metric in ["euclidean", "manhattan", "cosine"]:
        ...
        avg_weight = np.mean(graph[:, 2])
        print(
            f"{metric.capitalize()} metric: {len(graph)} edges, "
